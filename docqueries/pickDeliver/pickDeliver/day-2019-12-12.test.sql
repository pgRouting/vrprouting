...
FROM vrp_pickDeliverRaw(
    ...
    execution_date => EXTRACT(EPOCH FROM('2019-12-12 00:00:00'::TIMESTAMP))::BIGINT,
...
FROM vrp_pickDeliver(
    ...
    execution_date => '2019-12-12 00:00:00'::TIMESTAMP,
    optimize => true, factor => 1::float, stop_on_all_served => true, max_cycles => 1);
...
original_stops AS (
    ...
    FROM vehicles WHERE date_trunc('day', s_tw_open) = '2019-12-12 00:00:00'
)
SELECT count(*)
FROM original_stops
