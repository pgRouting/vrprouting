...
FROM vrp_pickDeliverRaw(
    $$SELECT * FROM shipments WHERE date_trunc('day', p_tw_open) = '2019-12-11 00:00:00'$$,
    $$SELECT * FROM vehicles  WHERE date_trunc('day', s_tw_open) = '2019-12-11 00:00:00'$$,
...
FROM vrp_pickDeliver(
    ...
    $$SELECT * FROM tdm('2019-12-11'::TIMESTAMP)$$,
    execution_date => '2019-12-11 00:00:00'::TIMESTAMP,
    optimize => true, factor => 1::float, stop_on_all_served => true, max_cycles => 1);
...
original_stops AS (
    ...
    FROM vehicles WHERE date_trunc('day', s_tw_open) = '2019-12-11 00:00:00'
)
SELECT count(*)
FROM original_stops
