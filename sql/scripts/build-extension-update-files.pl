...
my $version_0_2 = qr/(0.2.[\d+])/;
my $version_0_3 = qr/(0.3.[\d+])/;
my $version_0_4 = qr/(0.4.[\d+])/;
my $version_0_5 = qr/(0.5.[\d+])/;
# add minor here
...
sub generate_upgrade_script {
    ...
    if ($new_minor != $old_minor) {
        for my $old_function (sort @{$old_signatures}) {
            ...
            # On mayor 0, anything can happen
            die "$old_function should exist in $new_minor" if $new_mayor == $old_mayor && $new_mayor != 0;
