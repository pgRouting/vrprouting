...
CREATE OR REPLACE FUNCTION vrp_pickDeliverAddRaw(
  TEXT,   -- orders SQL
  TEXT,   -- vehicles SQL
...
BEGIN
  ...
  FROM vrp_compatibleVehiclesRaw($1, $2, $3, $4, $5, factor, dryrun => false) a
  INTO compatible_vehicles_arr;

  -- Where there compatible vehicles? no -> exception -> exit
...
COMMENT ON FUNCTION vrp_pickDeliverAddRaw(TEXT, TEXT, TEXT, TEXT, BIGINT, BIGINT, BOOLEAN, FLOAT, INTEGER, BOOLEAN)
IS 'vrp_pickDeliverAddRaw
- Documentation:
  - ${PROJECT_DOC_LINK}/vrp_pickDeliverAddRaw.html';
